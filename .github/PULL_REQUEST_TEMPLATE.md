<!--
  Publishing a cartridge to the registry? Fill this in. CI validates the
  listing automatically and comments the result. A human merges; nothing is
  merged by automation.
-->

## Listing

- **Name** (`registry/<slug>` or `<org>/<slug>`):
- **Hosting URL** (where the `.cart` bytes live, https, not cartridge.app):
- **License** (SPDX id from the allowlist in POLICY.md):
- **Source** (where the underlying material came from):
